...
class opts(object):
    ...
    index = False
    vacuum = False
    create_raster_overviews_table = False
    ...
    @classmethod
    def addRastFile(cls, infile):
        cls.raster.append(infile)
...
def raster2pgsql(rastfile, database, table, host="127.0.0.1", port=5432, user="postgres"):
    global VERBOSE
    ...
    with psycopg2.connect(database=database, user=user, password="", host=host, port=port) as conn:
        with conn.cursor() as curs:
            opts.output.seek(0)
            curs.execute(opts.output.read())
            opts.output.close()

if __name__ == "__main__":
    raster2pgsql("/mnt/f25/mapserver/mws/mws_2.tiff", "MapTileData", "cluster_data")
